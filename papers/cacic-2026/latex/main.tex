...
de las universidades nacionales aplicando estados de di\'alogo con procedencia}
...
\email{\{jmfernandez,ftamasi,smarchetti, moloriz\}@unlu.edu.ar}}
...
desplegado sobre los 21.364 actos administrativos de la Universidad Nacional de
...
prevalece y no se sobrescribe. En la evaluaci\'on, la fusi\'on resuelve por
completo las consultas por identificador (R@5 de 1{,}00) y el procesamiento
conversacional lleva la repregunta el\'iptica de 0{,}12 a 0{,}93; el 97{,}0\%
de los actos citados en las respuestas queda anclado en el material recuperado.
...
(RAG)~\cite{lewis2020rag} sobre el m\'odulo de publicaci\'on documental de SUDOCU,
implementado y evaluado sobre la totalidad de los actos publicados por la
Universidad Nacional de Luj\'an (21.364 documentos) y desplegado adem\'as sobre
...
autoadministrada; (ii) un estado de di\'alogo cuya \emph{procedencia} determina el
r\'egimen de recuperaci\'on, visible y editable por quien consulta, con
ablaci\'on de cada mecanismo; y (iii)
...
texto, los metadatos y un \'indice l\'exico FTS5\footnote{\url{https://sqlite.org/fts5.html}},
...
En cada turno, la consulta se reescribe como una consulta autocontenida,
condicionada a los \'ultimos intercambios mediante un modelo de lenguaje con
salida estructurada; la b\'usqueda opera sobre la consulta reescrita, y el estado
de di\'alogo complementa esa reescritura con la informaci\'on que el texto no
transporta.
...
El estado es visible sobre el campo de escritura y editable: quien consulta puede
corregir la entidad, descartar un acto o reincorporarlo. Lo descartado permanece
...
calidad autom\'atico que descart\'o el 34{,}5\%) y \emph{conversacionales} de dos
...
fragmento citado\footnote{En todos los experimentos el modelo generador de
respuestas fue \texttt{gpt-4o-mini} a temperatura 0.}.
...
completitud y claridad, con un bloque final de confianza, ventaja percibida y
disposici\'on a la adopci\'on.
...
H\'ibrido (sin estado) & 1.00 & 1.00 & 1.00 & 0.82 & 0.72 & 0.75 & 0.12 & 0.08 & 0.09 \\
...
h\'ibrido con anclaje resuelve por completo los identificadores y conserva la
ventaja en las de contenido. La columna conversacional muestra el papel del
...
condicionada al historial como mecanismo determinante; los \emph{slots} no desplazan estas
m\'etricas, y su funci\'on es dar a quien consulta un mecanismo de correcci\'on
cuya prioridad garantiza la procedencia. Un \emph{cross-encoder}
...
dimensi\'on sobre los \'items fijos (tres jueces) y bloque global.}
\label{tab:jueces}
\begin{tabular}{lc}
\toprule
...
acceso a los evaluadores---, la portabilidad se verific\'o contra el portal
SUDOCU de la Universidad Nacional de San Luis. Se gener\'o una
...
argentino---; y trazabilidad verificada de cada afirmaci\'on hasta el documento
oficial publicado.
...
iniciativa IPAC (C\'omputo de Alto Desempe\~no Argentina), bajo la convocatoria
PCI-146 (Proyectos de C\'omputo Intensivo), un programa nacional de
asignaci\'on de recursos de c\'omputo de alto rendimiento. Los autores
agradecen asimismo al Sistema Nacional de C\'omputo de Alto Desempe\~no
(SNCAD), a la infraestructura Clementina XXI, a la SICyT y a su equipo
t\'ecnico por el apoyo en la ejecuci\'on de los experimentos.
